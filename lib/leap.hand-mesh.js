  ...
  var ARM_TOP_AND_BOTTOM_ROTATION = (new THREE.Quaternion())
  ...
  HandMesh.prototype.createFingers = function () {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      for (var j = 0; j < boneCount; j++) {
        mesh = new THREE.Mesh(new THREE.SphereGeometry(jointRadius, 32, 32), this.material.clone());
        mesh.name = 'hand-bone-' + j;
        mesh.material.color.copy(options.jointColor);
        // mesh.renderDepth = ((i * 9) + (2 * j)) / 36;
        this.object3D.add(mesh);
        finger.push(mesh);

        mesh = new THREE.Mesh(
          new THREE.CylinderGeometry(boneRadius, boneRadius, 40, 32), this.material.clone()
        );
        mesh.name = 'hand-joint-' + j;
        mesh.material.color.copy(options.boneColor);
  ...
  HandMesh.prototype.createArm = function () {
    ...
    for (var i = 0; i <= 3; i++) {
      this.armBones.push(
        new THREE.Mesh(
          new THREE.CylinderGeometry(boneRadius, boneRadius, (i < 2 ? 1000 : 100), 32),
          this.material.clone()
        )
      );
      this.armBones[i].material.color.copy(options.boneColor);
    ...
    this.armSpheres = [];
    for (i = 0; i <= 3; i++) {
      this.armSpheres.push(
        new THREE.Mesh(new THREE.SphereGeometry(jointRadius, 32, 32), this.material.clone())
      );
      this.armSpheres[i].material.color.copy(options.jointColor);
